...
export default function Nifty() {
  ...
  const [numberOfGraphEntries, setNumberOfGraphEntries] = useState(5);
  const [inputValueGraph, setInputValueGraph] = useState(numberOfGraphEntries);
  const [graphData, setGraphData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(`https://nifty-signal-generation-tiqvavprfq-el.a.run.app/v1/nifty/priceaction/${numberOfGraphEntries}`);
      if (response.ok) {
  ...
  return (
    <>
      ...
      <div className="mb-4">
      <label for="entries" style={{ fontWeight: 'bold' }}>Select number of graph entries to display:</label>
      <input type="number" value={inputValueGraph} onChange={(e) => setInputValueGraph(e.target.value)} />
      </div>
      <LineChart
      width={500}
      height={300}
