...
export default function Nifty() {
  ...
  useEffect(() => {
    axios.get('https://nifty-signal-generation-tiqvavprfq-el.a.run.app/v1/nifty/priceaction')
      .then(response => {
  ...
  return (
    <>
      ...
      <div style={{ width: 700, height: 385 }}>
        <LineChart width={700} height={385} data={filteredData}>
          <Line type="monotone" dataKey={selectedData} stroke="#8884d8" />
          <XAxis dataKey="Date" />
          <YAxis />
      ...
      <div style={{ marginTop: 20 }}>
        <DropdownButton id="select-data" title="Select Data">
          <Dropdown.Item eventKey="Close">Close</Dropdown.Item>
          <Dropdown.Item eventKey="3EMA_Close">3EMA Close</Dropdown.Item>
          <Dropdown.Item eventKey="6EMA_Close">6EMA Close</Dropdown.Item>
          <Dropdown.Item eventKey="9EMA_Close">9EMA Close</Dropdown.Item>
        </DropdownButton>
        <ButtonGroup style={{ marginLeft: 20 }}>
          <Button variant="primary" onClick={() => handleFilterData(3)}>3 months</Button>
          <Button variant="primary" onClick={() => handleFilterData(6)}>6 months</Button>
          <Button variant="primary" onClick={() => handleFilterData(12)}>1 year</Button>
          <Button variant="primary" onClick={() => handleFilterData(null)}>All</Button>
        </ButtonGroup>
      </div>
    </>
  );
